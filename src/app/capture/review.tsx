...
import { extraPhotoCount, hasFrontPhoto, visiblePhotos, type EditableDraftKey } from '@/lib/draft';
...
export default function CaptureReviewScreen() {
  ...
  function onSubmit() {
    ...
    setPrompt('Submit to Firestore lands in a later phase. This draft is saved on the device.');
  }
      ...
      <ScrollView
        ...
        <View style={{ gap: Spacing.three }}>
          <ThemedText type="small" themeColor="textSecondary">
            {draft.photos.length} photo{draft.photos.length === 1 ? '' : 's'} in this draft
            {extraPhotoCount(draft) > 0 ? `, including ${extraPhotoCount(draft)} extra` : ''}. Tap a
            photo to open it full-size and confirm it. Use Zoom and crop if the label is small.
          </ThemedText>
            ...
            <View style={{ flexDirection: 'row', gap: Spacing.two }}>
              {sourcePhotos.map((photo) => (
                <Pressable
                  ...
                  onPress={() =>
                    router.push(`/capture/preview?photoId=${encodeURIComponent(photo.id)}`)
                  }>
                  <Image
                    ...
                    cachePolicy="none"
                    contentFit="cover"
                    style={{ width: 88, height: 88, borderRadius: 8, backgroundColor: '#222' }}
                  />
                  <ThemedText type="small" themeColor="textSecondary">
                    {photo.purpose}
                  </ThemedText>
          ...
          {isComplete ? null : (
            <ThemedText type="small">
              Username or PIN is missing. Save both in Settings, then return to Submit.
            </ThemedText>
          )}
          ...
          />
          <ThemedText type="small" themeColor="textSecondary">
            Check uploads photos and fills empty label fields. Fields you typed stay as-is on a later Check.
            Quantity is never filled from the label.
          </ThemedText>
          <UiButton label="Submit" onPress={onSubmit} />
          {isComplete ? null : (
            <UiButton label="Open Settings" variant="outlined" onPress={() => router.push('/settings')} />
          )}
